...
<div class="grid gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600 md:grid-cols-4">
    <div class="flex items-center gap-3">
        <a class="font-semibold text-emerald-600 hover:text-emerald-500" href="{{ route('question.show', ['question' => $post->id]) }}">share</a>
        <span class="text-slate-400">|</span>
        <a class="font-semibold text-emerald-600 hover:text-emerald-500" href="{{ route('question.edit', ['question' => $post->id]) }}">edit</a>
        <span class="text-slate-400">|</span>
        <a class="font-semibold text-emerald-600 hover:text-emerald-500" href="{{ route('question.show', ['question' => $post->id]) }}#flag">flag</a>
    </div>
    <div></div>
    <div>
        @if($post->last_editor_user_id != "")
            <div class="rounded-xl bg-white p-4 shadow-sm ring-1 ring-slate-900/5">
                ...
                <a href="{{ route('users.show', ['user' => $post->user_id]) }}" class="font-semibold text-emerald-600 hover:text-emerald-500">

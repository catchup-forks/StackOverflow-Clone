                ...
                <tr class="transition hover:bg-slate-50/80">
                    ...
                    <td class="p-4 text-sm text-slate-700">
                        <p class="font-semibold text-slate-900">
                            <a href="/question/{{ $q->id }}" class="hover:text-emerald-600">{{ $q->title }}</a>
                        </p>
                        <p class="mt-2 flex flex-wrap items-center gap-3 text-xs text-slate-500">
                            <span class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-700">Tag</span>
                            <span class="inline-flex items-center gap-1 rounded-full bg-emerald-100 px-3 py-1 font-medium text-emerald-700">Tags</span>
                            <span class="ml-auto font-semibold text-slate-700">{{ $q->user->reputation }}</span>
                            <a class="font-semibold text-emerald-600 hover:text-emerald-500" href="/user/{{ $q->user_id }}">{{ $q->user->display_name }}</a>
                        </p>
                    </td>
                </tr>
            @endforeach

...
<form action="/answer" method="POST" class="mt-6 space-y-4">
    @csrf
    <input type="hidden" name="post_id" value="{{ $post->id }}">
    <div class="space-y-2">
        <label for="answer" class="text-sm font-semibold text-slate-700">Your Answer</label>
        <textarea id="answer" name="answer" required class="min-h-[200px] w-full rounded-2xl border border-slate-300 bg-white p-4 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-emerald-500"></textarea>
    </div>
